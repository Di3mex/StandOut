...
public abstract class StandOutWindow extends Service
{
	...
	/**
	 * StandOut window id: You may use this id when you want it to be disregarded. The system makes no distinction for
	 * this id; it is only used to improve code readability.
	 */
	...
	/**
	 * Intent action: Restore a previously hidden window corresponding to the id. The window should be previously hidden
	 * with {@link #ACTION_HIDE}.
	 */
	...
	/**
	 * Intent action: Hide an existing window with an existing id. To enable the ability to restore this window, make
	 * sure you implement {@link #getHiddenNotification(int)}.
	 */
	 ...
	 * @param cls
	 *            The Service extending {@link StandOutWindow} that will be used to create and manage the window.
	 * @param id
	 *            The id representing this window. If the id exists, and the corresponding window was previously hidden,
	 *            then that window will be restored.
	 * 
	 * @see #show(int)
	 */
	...
	/**
	 * Hide the existing window corresponding to the id. To enable the ability to restore this window, make sure you
	 * implement {@link #getHiddenNotification(int)}.
	 * 
	 * @param context
	 *            A Context of the application package implementing this class.
	 * @param cls
	 *            The Service extending {@link StandOutWindow} that is managing the window.
	 * @param id
	 *            The id representing this window. The window must previously be shown.
	 * @see #hide(int)
	 */
	 ...
	 * 
	 * <p>
	 * Send {@link Parceleable} data in a {@link Bundle} to a new or existing windows. The implementation of the
	 * recipient window can handle what to do with the data. To receive a result, provide the class and id of the
	 * sender.
	 * 
	 * @param context
	 *            A Context of the application package implementing the class of the sending window.
	 * @param toCls
	 *            The Service's class extending {@link StandOutWindow} that is managing the receiving window.
	 * @param toId
	 *            The id of the receiving window, or DISREGARD_ID.
	 * @param requestCode
	 *            Provide a request code to declare what kind of data is being sent.
	 * @param data
	 *            A bundle of parceleable data to be sent to the receiving window.
	 * @param fromCls
	 ...
	 *            A bundle of parceleable data to be sent to the receiving window.
	 * @param fromCls
	 *            If the sending window wants a result, provide the class of the sending window.
	 * @param fromId
	 *            If the sending window wants a result, provide the id of the sending window.
	 * @return An {@link Intnet} to use with {@link Context#startService(Intent)}.
	 */
	...
	/**
	 * Return the name of every window in this implementation. The name will appear in the default implementations of
	 * the system window decoration title and notification titles.
	 * 
	 * @return The name.
	 */
	...
	/**
	 * Return the icon resource for every window in this implementation. The icon will appear in the default
	 * implementations of the system window decoration and notifications.
	 * 
	 * @return The icon.
	 */
	...
	/**
	 * Create a new {@link View} corresponding to the id, and add it as a child to the frame. The view will become the
	 * contents of this StandOut window. The view MUST be newly created, and you MUST attach it to the frame.
	 * 
	 * <p>
	 * If you are inflating your view from XML, make sure you use
	 * {@link LayoutInflater#inflate(int, ViewGroup, boolean)} to attach your view to frame. Set the ViewGroup to be
	 * frame, and the boolean to true.
	 * 
	 * <p>
	 * If you are creating your view programmatically, make sure you use {@link FrameLayout#addView(View)} to add your
	 * view to the frame.
	 * 
	 * @param id
	...
	/**
	 * Return the {@link StandOutWindow#LayoutParams} for the corresponding id. The system will set the layout params on
	 * the view for this StandOut window. The layout params may be reused.
	 * 
	 * 
	 * @param id
	 *            The id of the window.
	 * @param window
	 *            The window corresponding to the id. Given as courtesy, so you may get the existing layout params.
	 * @return The {@link StandOutWindow#LayoutParams} corresponding to the id. The layout params will be set on the
	 *         window. The layout params returned will be reused whenever possible, minimizing the number of times
	 *         getParams() will be called.
	 */
	 ...
	 * 
	 * <p>
	 * You may use any of the flags defined in {@link StandOutFlags}. This method will be called many times, so keep it
	 * fast.
	 * 
	 * <p>
	 * Use bitwise OR (|) to set flags, and bitwise XOR (^) to unset flags. To test if a flag is set, use
	 * {@link Utils#isSet(int, int)}.
	 * 
	 * @param id
	 *            The id of the window.
	...
	/**
	 * Return the title for the persistent notification. This is called every time {@link #show(int)} is called.
	 * 
	 * @param id
	...
	/**
	 * Return the message for the persistent notification. This is called every time {@link #show(int)} is called.
	 * 
	 * @param id
	 *            The id of the window shown.
	...
	/**
	 * Return the intent for the persistent notification. This is called every time {@link #show(int)} is called.
	 * 
	 * <p>
	 * The returned intent will be packaged into a {@link PendingIntent} to be invoked when the user clicks the
	 * notification.
	 * 
	 * @param id
	 *            The id of the window shown.
	...
	/**
	 * Return the icon resource for every hidden window in this implementation. The icon will appear in the default
	 * implementations of the hidden notifications.
	 * 
	 * @return The icon.
	 */
	...
	/**
	 * Return a persistent {@link Notification} for the corresponding id. You must return a notification for AT LEAST
	 * the first id to be requested. Once the persistent notification is shown, further calls to
	 * {@link #getPersistentNotification(int)} may return null. This way Android can start the StandOut window service
	 * in the foreground and will not kill the service on low memory.
	 * 
	 * <p>
	 * As a courtesy, the system will request a notification for every new id shown. Your implementation is encouraged
	 * to include the {@link PendingIntent#FLAG_UPDATE_CURRENT} flag in the notification so that there is only one
	 * system-wide persistent notification.
	 * 
	 * <p>
	 * See the StandOutExample project for an implementation of {@link #getPersistentNotification(int)} that keeps one
	 * system-wide persistent notification that creates a new window on every click.
	 * 
	 * @param id
	 *            The id of the window.
	 * @return The {@link Notification} corresponding to the id, or null if you've previously returned a notification.
	 */
	...
	/**
	 * Return a hidden {@link Notification} for the corresponding id. The system will request a notification for every
	 * id that is hidden.
	 * 
	 * <p>
	 * If null is returned, StandOut will assume you do not wish to support hiding this window, and will
	 * {@link #close(int)} it for you.
	 * 
	 * <p>
	 * See the StandOutExample project for an implementation of {@link #getHiddenNotification(int)} that for every
	 * hidden window keeps a notification which restores that window upon user's click.
	 * 
	 * @param id
	 *            The id of the window.
	...
	/**
	 * You probably want to leave this method alone and implement {@link #getDropDownItems(int)} instead. Only implement
	 * this method if you want more control over the drop down menu.
	 * 
	 * <p>
	 * Implement this method to set a custom drop down menu when the user clicks on the icon of the window corresponding
	 * to the id. The icon is only shown when {@link StandOutFlags#FLAG_DECORATION_SYSTEM} is set.
	 * 
	 * @param id
	 *            The id of the window.
	...
	public PopupWindow getDropDown(final int id)
	{
		...
		items.add(new DropDownListItem(android.R.drawable.ic_menu_close_clear_cancel, "Quit " + getAppName(),
				new Runnable()
				{
	...
	/**
	 * Implement this method to populate the drop down menu when the user clicks on the icon of the window corresponding
	 * to the id. The icon is only shown when {@link StandOutFlags#FLAG_DECORATION_SYSTEM} is set.
	 * 
	 * @param id
	 *            The id of the window.
	 * @return The list of items to show in the drop down menu, or null or empty to have no dropdown menu.
	 */
	...
	/**
	 * Implement this method to be alerted to touch events in the body of the window corresponding to the id.
	 * 
	 * <p>
	 * Note that even if you set {@link #FLAG_DECORATION_SYSTEM}, you will not receive touch events from the system
	 * window decorations.
	 * 
	...
	/**
	 * Implement this callback to be alerted when a window corresponding to the id is about to be shown. This callback
	 * will occur before the view is added to the window manager.
	 * 
	 * @param id
	 *            The id of the view, provided as a courtesy.
	 * @param view
	...
	/**
	 * Implement this callback to be alerted when a window corresponding to the id is about to be hidden. This callback
	 * will occur before the view is removed from the window manager and {@link #getHiddenNotification(int)} is called.
	 * 
	 * @param id
	 *            The id of the view, provided as a courtesy.
	 * @param view
	...
	/**
	 * Implement this callback to be alerted when a window corresponding to the id is about to be closed. This callback
	 * will occur before the view is removed from the window manager.
	 * 
	 * @param id
	 *            The id of the view, provided as a courtesy.
	 * @param view
	...
	/**
	 * Implement this callback to be alerted when all windows are about to be closed. This callback will occur before
	 * any views are removed from the window manager.
	 * 
	...
	/**
	 * Implement this callback to be alerted when a window corresponding to the id has received some data. The sender is
	 * described by fromCls and fromId if the sender wants a result. To send a result, use
	 * {@link #sendData(int, Class, int, int, Bundle)}.
	 * 
	 * @param id
	 *            The id of your receiving window.
	 * @param requestCode
	 *            The sending window provided this request code to declare what kind of data is being sent.
	 * @param data
	 *            A bundle of parceleable data that was sent to your receiving window.
	 * @param fromCls
	 *            The sending window's class. Provided if the sender wants a result.
	 * @param fromId
	 *            The sending window's id. Provided if the sender wants a result.
	 */
	...
	/**
	 * Implement this callback to be alerted when a window corresponding to the id is about to be updated in the layout.
	 * This callback will occur before the view is updated by the window manager.
	 * 
	 * @param id
	...
	/**
	 * Implement this callback to be alerted when a window corresponding to the id is about to be bought to the front.
	 * This callback will occur before the window is brought to the front by the window manager.
	 * 
	 * @param id
	 *            The id of the window, provided as a courtesy.
	 * @param view
	 ...
	 * This callback will occur before the window's focus is changed.
	 * 
	 * @param id
	 *            The id of the window, provided as a courtesy.
	 * @param view
	...
	/**
	 * Implement this callback to be alerted when a window corresponding to the id receives a key event. This callback
	 * will occur before the window handles the event with {@link Window#dispatchKeyEvent(KeyEvent)}.
	 * 
	 * @param id
	 *            The id of the window, provided as a courtesy.
	 * @param view
	 ...
	 * @param event
	 *            The key event.
	 * @return Return true to cancel the window from handling the key event, or false to let the window handle the key
	 *         event.
			...
			else
			{
				...
				if (!startedForeground)
				{
					throw new RuntimeException("Your StandOutWindow service must"
							+ "provide a persistent notification." + "The notification prevents Android"
							+ "from killing your service in low" + "memory situations.");
				}
			}
		}
	...
	/**
	 * Send {@link Parceleable} data in a {@link Bundle} to a new or existing windows. The implementation of the
	 * recipient window can handle what to do with the data. To receive a result, provide the id of the sender.
	 * 
	 * @param fromId
	 *            Provide the id of the sending window if you want a result.
	 * @param toCls
	...
	/**
	 * Bring the window corresponding to this id in front of all other windows. The window may flicker as it is removed
	...
	/**
	 * Request focus for the window corresponding to this id. A maximum of one window can have focus, and that window
	 * will receive all key events, including Back and Menu.
	 * 
	 * @param id
	 *            The id of the window.
	...
	/**
	 * Remove focus for the window corresponding to this id. Once a window is unfocused, it will stop receiving key
	 * events.
	 * 
	 * @param id
	 *            The id of the window.
	...
	/**
	 * Courtesy method for your implementation to use if you want to. Gets a unique id to assign to a new window.
	 * 
	...
	/**
	 * Return whether the window corresponding to the id exists. This is useful for testing if the id is being restored
	 * (return true) or shown for the first time (return false).
	 * 
	 * @param id
	 *            The id of the window.
	 * @return True if the window corresponding to the id is either shown or hidden, or false if it has never been shown
	...
	/**
	 * Return the window corresponding to the id, if it exists in cache. The window will not be created with
	 * {@link #createAndAttachView(int, ViewGroup)}. This means the returned value will be null if the window is not
	 * shown or hidden.
	 * 
	 * @param id
	 *            The id of the window.
	...
	/**
	 * Return whole window cache
	 * 
	 * @return
	 */
	public static final WindowCache getWindowCache()
	{
		return sWindowCache;
	}
	 ...
	 * {@link StandOutFlags#FLAG_DECORATION_SYSTEM} is set, or if your own view contains a TextView with id R.id.title.
	 * 
	 * @param id
	 *            The id of the window.
	 * @param text
	 ...
	 * {@link StandOutFlags#FLAG_DECORATION_SYSTEM} is set, or if your own view contains a TextView with id
	 * R.id.window_icon.
	 * 
	 * @param id
	 *            The id of the window.
	...
	/**
	 * Remove focus for the window, which could belong to another application. Since we don't allow windows from
	 * different applications to directly interact with each other, except for
	 * {@link #sendData(Context, Class, int, int, Bundle, Class, int)}, this method is private.
	 * 
	 * @param window
	...
	public class StandOutLayoutParams extends WindowManager.LayoutParams
	{
		...
		{
			super(200, 200, TYPE_PHONE, StandOutLayoutParams.FLAG_NOT_TOUCH_MODAL
					| StandOutLayoutParams.FLAG_WATCH_OUTSIDE_TOUCH, PixelFormat.TRANSLUCENT);
		 ...
		 *            The mininum height of the window.
		 * @param threshold
		 *            The touch distance threshold that distinguishes a tap from a drag.
		 */
